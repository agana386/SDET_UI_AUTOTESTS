...
@allure.title("TC001: Случайная категория → ≥4 товара → все сортировки")
...
class TestCategorySorting:
    ...
    def test_sorting_all_variants(self):
        ...
        with allure.step(f"Шаг 2: Проверить ≥4 товаров в «{cat_name}»"):
            ...
            assert len(names) >= 4, f"«{cat_name}»: {len(names)} товаров, нужно ≥4"

        with allure.step("Шаг 3: Name A-Z"):
            ...
            assert names_asc == expected, \
                f"[{cat_name}] A-Z сломан.\nПолучено: {names_asc}\nОжидалось: {expected}"
            self._screenshot("03_az")

        with allure.step("Шаг 4: Name Z-A"):
            ...
            assert names_desc == expected, \
                f"[{cat_name}] Z-A сломан.\nПолучено: {names_desc}\nОжидалось: {expected}"
            self._screenshot("04_za")

        with allure.step("Шаг 5: Price Low→High"):
            ...
            assert prices_asc == sorted(prices_asc), \
                f"[{cat_name}] Low→High сломан.\nПолучено: {prices_asc}\nОжидалось: {sorted(prices_asc)}"
            self._screenshot("05_price_asc")

        with allure.step("Шаг 6: Price High→Low"):
